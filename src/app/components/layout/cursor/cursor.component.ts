...
@Component({
  ...
  template: `
    <div
      #cursor
      class="fixed top-0 left-0 w-4 h-4 bg-black dark:bg-white rounded-full pointer-events-none z-[9999] opacity-0 transition-opacity duration-300"
    ></div>
    <div
      #follower
      class="fixed top-0 left-0 w-12 h-12 border border-black dark:border-white rounded-full pointer-events-none z-[9998] opacity-0 transition-opacity duration-300"
    ></div>
  `,
  styles: [
    `
...
export class CursorComponent implements AfterViewInit, OnDestroy {
  ...
  private initCursor() {
    ...
    this.cursorX = gsap.quickTo(cursorEl, 'x', {
      duration: 0.1,
      ease: 'power3',
    });
    this.cursorY = gsap.quickTo(cursorEl, 'y', {
      duration: 0.1,
      ease: 'power3',
    });

    this.followerX = gsap.quickTo(followerEl, 'x', {
      duration: 0.6,
      ease: 'power3',
    });
    this.followerY = gsap.quickTo(followerEl, 'y', {
      duration: 0.6,
      ease: 'power3',
    });
